    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="m-2 p-2 bg-slate-100 rounded">
                ...
                <form method="POST" action="{{ route('admin.tables.update', $table->id) }}" class="">
                    @csrf
                    @method('PUT')
                    <div class="mb-6">
                        <label for="name" class="block mb-2 text-m font-medium text-gray-900 dark:text-dark">
                            Name</label>
                        <input value="{{ $table->name }}" name="name" type="name" id="name"
                    ...
                    <div class="mb-6">
                        <label for="guest_number" class="block mb-2 text-m font-medium text-gray-900 dark:text-dark">
                            Guest Number</label>
                        <input name="guest_number" value="{{ $table->guest_number }}" type="number" id="guest_number"
                            class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-700 focus:border-indigo-700 block w-full p-2.5 dark:bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-dark dark:focus:ring-indigo-700 dark:focus:border-indigo-700"
                            placeholder="" required>
                    ...
                    <div class="mb-6">
                        ...
                        <select name="status" id="status"
                            ...
                            @foreach (App\Enums\TableStatus::cases() as $status)
                                <option @selected($table->status == $status->value) value="{{ $status->value }}">{{ $status->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-6">
                        ...
                        <select name="location" id="location"
                            ...
                            @foreach (App\Enums\TableLocation::cases() as $location)
                                <option @selected($table->location == $location->value) value="{{ $location->value }}">
